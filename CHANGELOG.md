...
- Added a mapped device profile for the Razer Kraken Kitty V2 headset (`0x0560`, `DeviceFormFactor.headset`), the first device routed through a bespoke `usesKrakenLegacyProtocol` path: a dedicated `KrakenLegacyProtocol` (EEPROM/RAM address-space commands) and `KrakenLegacyControlSession` (talking to the headset's single consumer-control USB HID interface, since it has no 90-byte Razer control interface) replace the shared class/cmd USB protocol for this device only. Ships lighting-only support (`off`, `static`, `spectrum`, single/dual-color breathing) with no DPI, poll-rate, power-management, or button-remap hardware. Adds `OpenSnekProbe usb-kraken-info` and `usb-kraken-effect` commands. Not yet validated on real hardware.
