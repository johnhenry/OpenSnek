...
- Added groundwork for legacy-protocol Razer Kraken headsets: a byte-exact `KrakenLegacyProtocol` module (OpenRazer's 37-byte EEPROM/RAM command scheme with tests), a `KrakenLegacyControlSession` transport, a `usesKrakenLegacyProtocol` profile path with a `DeviceFormFactor.headset`, `OpenSnekProbe usb-kraken-info` / `usb-kraken-effect` commands, and protocol docs. The Kraken Kitty V2 (`0x0560`) profile exists but is deliberately **not registered**: contributor hardware validation showed macOS cannot deliver the legacy protocol through IOHIDLib (descriptor-limited report sizes; no populated responses), so the device correctly remains on the unsupported/no-control-interface path. Findings recorded in `docs/research/KRAKEN_KITTY_V2_MACOS_TRANSPORT_FINDINGS.md`.
